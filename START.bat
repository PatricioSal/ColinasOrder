...
start "WhatsApp Bot — Python Webhook" cmd /k "title WhatsApp Bot — Python Webhook && color 0A && echo. && echo  [Python] WhatsApp Webhook starting on port 5050... && echo. && py src\whatsapp_webhook.py"

:: Brief pause so Python initialises first
timeout /t 3 /nobreak >nul

:: ----- Window 2: Node.js WhatsApp listener -----
start "WhatsApp Bot — Node Listener" cmd /k "title WhatsApp Bot — Node Listener && color 0B && echo. && echo  [Node.js] WhatsApp Listener starting on port 3000... && echo  (Scan the QR code in WhatsApp the first time) && echo. && node src\whatsapp_listener.js"
